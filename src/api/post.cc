/**
 * This serves as a supporting source file to implement
 * the postimage computation.
 *
 * @date   Nov 16, 2015
 * @author Peizun Liu
 */
...
/**
 * To compute all post images of global state tau: the implementation
...
/**
 * To compute all post images of global state tau: the post images are
...
void post_image::compute_post_images(const prog_state& tau,
	...
	for (auto ie = successors.cbegin(); ie != successors.cend(); ++ie) {
		...
		case type_stmt::IFEL: {
			...
			///
			/// SEMANTIC:
			const auto& cond = ie->get_stmt().get_condition().eval(sv, lv);
			if (cond == sool::T) {
				...
				const auto& _l1 = this->compute_image_ifth_stmt(l, _pc);
				images.emplace_back(s, alg::update_counters(_l1, l, Z));

				const auto& _l2 = this->compute_image_else_stmt(l);
				images.emplace_back(s, alg::update_counters(_l2, l, Z));
			}
		}
			break;
		case type_stmt::ASSE: {
		...
		case type_stmt::NTHR: {
			...
			/// increment l''.pc = label and decrement l
			auto _Z = alg::update_counters(local_state(_pc, lv), l, Z);
			/// increment l'.pc = l.pc + 1, update _Z...
			alg::increment(local_state(pc + 1, lv), _Z);
			images.emplace_back(s, _Z);
		}
			break;
		case type_stmt::ETHR: {
		...
		case type_stmt::ATOM: {
			...
			/// states after across atomic section
			const auto& sp = this->compute_image_atom_sect(sv, _pc, lv);
			for (const auto& _p : sp) {
				const auto& _Z = alg::update_counters(
						_p.get_l(), l, Z);
				images.emplace_back(_p.get_s(), _Z);
			}
		}
			break;
		case type_stmt::BCST: {
...
/**
 * Compute post images across an atomic section:
 ...
 */
deque<prog_thread> post_image::compute_image_atom_sect(const state_v sv,
		const size_pc pc, const state_v lv) {
	deque<prog_thread> result;
	queue<prog_thread> worklist;
	set<prog_thread> explored;

	worklist.emplace(sv, pc, lv);
	while (!worklist.empty()) {
		const auto t = worklist.front();
		worklist.pop();
		if (explored.find(t) != explored.end()) {
			cout << t << " has seen before...\n";
			continue;
		}
		explored.emplace(t);
		auto sv = t.get_s().get_vars();
		auto lv = t.get_l().get_vars();
		auto pc = t.get_l().get_pc();
		...
		for (auto ie = successors.cbegin(); ie != successors.cend(); ++ie) {
			...
			case type_stmt::ASSG: {
				...
				deque<state_v> lvs;
				this->compute_image_assg_stmt(sv, lv, pc, svs, lvs);
				const expr& cond = ie->get_stmt().get_condition();
				deque<pair<state_v, state_v>> tmp_result;
				for (const auto& _sv : svs) {
					for (const auto& _lv : lvs)
						if (cond.is_void()
								|| (cond.eval(sv, lv, _sv, _lv) != sool::F))
							worklist.emplace(_sv, _pc, _lv);
				}
			}
				break;
			case type_stmt::IFEL: {
				...
				const auto& cond = ie->get_stmt().get_condition().eval(sv, lv);
				if (cond == sool::T) {
					const auto& _l = this->compute_image_ifth_stmt(t.get_l(),
							_pc);
					worklist.emplace(t.get_s(), _l);
				} else if (cond == sool::F) {
					const auto& _l = this->compute_image_else_stmt(t.get_l());
					worklist.emplace(t.get_s(), _l);
				} else if (cond == sool::N) {
					const auto& _l1 = this->compute_image_ifth_stmt(t.get_l(),
							_pc);
					worklist.emplace(t.get_s(), _l1);

					const auto& _l2 = this->compute_image_else_stmt(t.get_l());
					worklist.emplace(t.get_s(), _l2);
				}
			}
				break;
			case type_stmt::ASSU: {
